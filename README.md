...
### 1️⃣ **Flashing du firmware**
Ouvre le web flasher dans Chrome/Edge et clique "Flasher" :
➡️ **[Web Flasher →](https://rdelfosse.github.io/vibe-m5stack/flash/)**
> ⚠️ Navigateur **Chromium** requis (Chrome/Edge) pour Web Serial API. Driver CP210x nécessaire sous Windows.
...
./install.sh
```
> Ces scripts installent automatiquement `uv`, `mistral-vibe`, et le plugin `vibe-m5stack`.

### 3️⃣ **Configurer le port**
```bash
vibe-m5stack setup
```
Détecte automatiquement ton M5Stack et sauvegarde la configuration.

### 4️⃣ **Lancer**
```bash
vibe-m5stack
```
Le plugin se lance avec Vibe et intercepte les demandes d'approbation.
...
<details>
<summary><strong>📦 Hardware requis</strong></summary>
...
<details>
<summary><strong>🛠️ Installation manuelle du firmware (PlatformIO) — si tu n'utilises pas le web flasher</strong></summary>
...
<details>
<summary><strong>🐍 Installer le plugin Python — méthode manuelle</strong></summary>

`mistral-vibe` est installé via `uv tool install mistral-vibe`, ce qui crée un venv
isolé dans `~\AppData\(Roaming\|Local\)\uv\tools\mistral-vibe\`. Le plugin `vibe-m5stack` doit
...
> `vibe-m5stack.exe` est bien créé dans le venv (`~\(AppData\Roaming\|Local\)\uv\tools\mistral-vibe\Scripts\`)
...
<details>
<summary><strong>🔧 Variables d'environnement obligatoires</strong></summary>
...
<details>
<summary><strong>👥 Plusieurs sessions Vibe en parallèle</strong></summary>
...
<details>
<summary><strong>⚙️ Comment ça marche — le hook approval</strong></summary>
...
<details>
<summary><strong>🎨 Configuration / personnalisation</strong></summary>
...
<details>
<summary><strong>📊 Statut ambiant + Watchdog (feature A+B)</strong></summary>
...
<details>
<summary><strong>🌐 API Mistral — endpoint d'usage manquant</strong></summary>
...
existant. Plus de dépendance à un cookie web fragile.

</details>

<details>
<summary><strong>⚠️ Limites connues</strong></summary>
...
<details>
<summary><strong>🚀 Roadmap / idées</strong></summary>
